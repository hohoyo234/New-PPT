...
export function initTracking() {
  ...
  const end = () => {
    if (document.visibilityState === 'hidden') {
      send(buildEvent('session_end', '', Math.round((Date.now() - sessionStart) / 1000)), true);
    }
  };
  document.addEventListener('visibilitychange', end);
  window.addEventListener('pagehide', () => send(buildEvent('session_end', '', Math.round((Date.now() - sessionStart) / 1000)), true));
}
