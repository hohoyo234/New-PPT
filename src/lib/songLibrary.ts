...
export interface LibrarySong {
  ...
  seed?: boolean;
  /** True when this entry is a community "社区精修版" (from the shared pool). */
  curated?: boolean;
}
...
// Merge the signed-in user's private cloud library DOWN into local storage.
// Purely additive: cloud entries are added or update the matching local entry by
// title, but nothing local (including the seeded SOP catalog) is ever removed.
// Does NOT call cloud hooks — this is a pull, we don't want to echo it back up.
export function mergeUserSongs(cloudSongs: LibrarySong[]): { added: number; updated: number } {
  const lib = loadLibrary();
  const byTitle = new Map(lib.map((e, i) => [normalize(e.title), i] as const));
  let added = 0, updated = 0;
  for (const s of cloudSongs) {
    ...
    const at = byTitle.get(key);
    if (at != null) {
      lib[at] = { ...lib[at], ...s, id: lib[at].id, seed: false };
      updated++;
    } else {
      lib.unshift({ ...s, seed: false });
      byTitle.set(key, 0);
      added++;
    }
  }
  try {
    localStorage.setItem(LIB_KEY, JSON.stringify(lib.slice(0, 2000)));
  } catch {}
  emitChange();
  return { added, updated };
}
