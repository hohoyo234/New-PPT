 ...
 *
 * Vendored (MIT) from TA3/web-user-behaviour so it ships in our bundle instead
 * of loading from a CDN at runtime. Only change vs upstream: the mouse-movement
 * sampling interval now reads user_config (upstream read defaults, ignoring the
 * configured value), and an ESM `export default` at the bottom.
 */
var userBehaviour = (function () {
    ...
    function start() {
        ...
        if (user_config.mouseMovement) {
            ...
            }, (user_config.mouseMovementInterval || defaults.mouseMovementInterval) * 1000);
        }
        //CLICKS
