...
export default function ManualMode({ modeToggle, authSlot }: { modeToggle: React.ReactNode; authSlot?: React.ReactNode }) {
  ...
  useEffect(() => { trackSearch(songQuery, songResults.length); }, [songQuery, songResults.length]);
  ...
  const handleLyricSheet = () => {
    ...
    const ok = openLyricSheet(langSongs, deckName);
        ...
        <section className="lg:col-span-3 space-y-5">
          <Panel title="背景">
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => { track('click', '上传背景'); uploadRef.current?.click(); }} className="flex-1 h-10 rounded-xl bg-[#F9F7F5] hover:bg-[#E5E0DA] text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all"><span className="material-symbols-outlined text-[16px]">upload</span>上传图</button>
              <button onClick={() => { track('click', 'AI背景'); setAiOpen(true); }} className="flex-1 h-10 rounded-xl bg-black text-white hover:bg-emerald-600 text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all"><span className="material-symbols-outlined text-[16px]">auto_awesome</span>AI 生成</button>
            </div>
