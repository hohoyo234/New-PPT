...
export default function LibraryMode({ modeToggle, authSlot }: { modeToggle: React.ReactNode; authSlot?: React.ReactNode }) {
  ...
  // Pending action gated behind an agreement modal: either opening a curated song
  // ('community') or contributing one to the pool ('contribute').
  const [gate, setGate] = useState<{ kind: 'community' | 'contribute'; run: () => void } | null>(null);
  ...
  // Debounced community-pool lookup. Surfaces 社区精修版 matches alongside the
  // local results while the user searches. Fails silently when offline.
  useEffect(() => {
    const term = q.trim();
    if (term.length < 2) { setCommunity([]); return; }
    let live = true;
    const t = window.setTimeout(() => {
      communitySearch(term).then((rows) => {
        if (!live) return;
        const haveLocal = new Set(songs.map((s) => (s.title || '').toLowerCase().replace(/\s+/g, '')));
        // Only show curated entries the user doesn't already have locally.
        setCommunity(rows.filter((r) => r.curated && !haveLocal.has((r.title || '').toLowerCase().replace(/\s+/g, ''))));
      });
    }, 350);
    return () => { live = false; window.clearTimeout(t); };
  }, [q, songs]);

  // Import a curated song into the local library (gated by the 共享互助协议).
  const importCurated = (s: LibrarySong) => {
    saveToLibrary({ title: s.title, titleSc: s.titleSc, englishTitle: s.englishTitle, producer: s.producer, lyrics: s.lyrics, lyricsSc: s.lyricsSc, englishLyrics: s.englishLyrics, bg: s.bg });
    refresh();
    setCommunity((prev) => prev.filter((c) => c.id !== s.id));
    flash('✅ 已加入歌库');
  };

  const openCurated = (s: LibrarySong) => {
    if (hasConsented('community')) importCurated(s);
    else setGate({ kind: 'community', run: () => importCurated(s) });
  };

  // Contribute a song to the community 精修库 (gated by the 内容合规协议).
  const doContribute = async (s: LibrarySong) => {
    flash('⏳ 正在上传…', 60000);
    try {
      await contributeCurated(s, user?.id);
      flash('🤝 已贡献至社区精修库，感谢分享！');
    } catch {
      flash('❌ 上传失败，请稍后再试');
    }
  };

  const contribute = (s: LibrarySong) => {
    track('click', '贡献社区');
    if (!user) { openAuth(); return; }
    if (!s.lyrics?.trim()) { flash('❌ 仅歌名无法贡献，请先补全歌词'); return; }
    if (hasConsented('contribute')) doContribute(s);
  ...
  useEffect(() => { trackSearch(q, filtered.length); }, [q, filtered.length]);
  ...
  return (
    <div className="min-h-full flex flex-col">
      ...
      <main className="flex-1 max-w-[1100px] w-full mx-auto px-5 lg:px-10 py-8">
        ...
        {/* 社区精修版 — curated matches from the shared pool, gated by agreement */}
        {community.length > 0 && (
          <div className="mb-5 rounded-3xl border border-violet-200/70 bg-gradient-to-br from-violet-50/80 to-white shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-violet-100">
              <span className="material-symbols-outlined text-violet-500 text-[20px]">diversity_3</span>
              <span className="font-serif font-black text-[#2C2C2C]">社区精修版</span>
              <span className="text-[10px] font-bold text-violet-500/80 bg-violet-100 px-2 py-0.5 rounded-full">同工分享 · {community.length}</span>
            </div>
            <div className="divide-y divide-violet-100/70">
              {community.map((s) => (
                <div key={s.id} className="flex items-center gap-3 px-5 py-3.5">
                  <span className="material-symbols-outlined text-violet-400 text-[20px] shrink-0">auto_awesome</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-serif font-black text-[#2C2C2C] truncate">{s.title || '未命名'}</span>
                      <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded bg-violet-100 text-violet-700 shrink-0">社区精修版</span>
                    </div>
                    {s.englishTitle && <span className="text-[11px] text-outline/40 font-medium truncate">{s.englishTitle}</span>}
                  </div>
                  <button onClick={() => openCurated(s)} className="h-9 px-4 rounded-xl bg-violet-600 text-white text-[10px] font-black uppercase tracking-wider hover:bg-violet-500 shadow flex items-center gap-1.5 shrink-0">
                    <span className="material-symbols-outlined text-[16px]">visibility</span>查看精修内容
                  </button>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-3xl border border-[#E5E0DA]/50 shadow-sm divide-y divide-[#E5E0DA]/40 overflow-hidden">
          {filtered.length === 0 && community.length === 0 && <div className="py-20 text-center text-outline/40 font-bold text-sm">没有匹配的歌曲</div>}
          {filtered.map((s, i) => {
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[32px] shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <div className="mt-3"><L label="英文歌词 / 对照(可选)"><textarea value={editing.englishLyrics || ''} onChange={(e) => setEditing({ ...editing, englishLyrics: e.target.value })} rows={5} className="li resize-none leading-relaxed" /><span className="block text-[10px] font-medium text-outline/40 px-1 mt-1 leading-snug">内置英文翻译为参考版本、非官方译本，可能与会众熟悉的唱本不同，敬拜前请自行核对或修改。</span></L></div>
