...
> 安全提示:`supabase_ppt_library.sql` 已默认「任何人可读、仅登录用户可写」。若你的 Supabase 里之前跑过旧版本(允许匿名写),请补跑一次 `supabase_rls_hardening.sql` 收回匿名的删/改权限。
...
npm test           # 运行核心逻辑冒烟测试(Vitest)
```

`npm test` 覆盖生成引擎的纯函数:分页 `paginateLyrics`、段落展开 `expandSongSections`、副歌识别 `detectChorus`、条目解析 `parseEntryLine` 等,改动这些逻辑时可快速发现回归。
